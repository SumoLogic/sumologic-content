...
# add the following timestamp parse rule to the Sumo source: yyyyMMddHHmmss.SSS

globs = {
	"access_token": "", #UPDATE ME
	"sumo_collector_url": "", # UPDATE ME
	"client_id": "", # UPDATE ME
	"refresh_token": "", # UPDATE ME
	"refresh_url": "https://test.salesforce.com/services/oauth2/token",
	"ssm": None,
	"sfdc_url": f"https://{sfdc_instance}.salesforce.com/services/data/v32.0/",
	"sfdc_instance": "" # UPDATE ME
}
...
def getSfdcLogsForEventType(event_type, date):
	...
	url = f"{globs["sfdc_url"]}query?q=SELECT+Id+,+EventType+,+LogFile+,+CreatedDate+,+LogFileLength+FROM+EventLogFile+WHERE+CreatedDate+>+{date}+AND+EventType+=+'{event_type}'" 
	response = httpGet(url)
